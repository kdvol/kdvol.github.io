...
CONFIG = {
    "telegram_chat": "soonsalchat",   # 실시간 대화(양방향 그룹)
    "telegram": "soonsal",            # 브로드캐스트 채널(팔로우)
...
<p>장 흐름, 속보 반응, 서로의 포지션 — 실시간 대화는 텔레그램 오픈채팅에서 오갑니다. 지금 참여하세요.</p>
<a class="btn" href="https://t.me/__TGCHAT__" target="_blank" rel="noopener">텔레그램 대화방 참여 →</a>
...
def build():
    ...
    html = (PAGE.replace("__BASE__", BASE)
            .replace("__TGCHAT__", CONFIG["telegram_chat"])
            .replace("__TG__", CONFIG["telegram"])
            .replace("__IG__", CONFIG["instagram"]))
    (OUT / "index.html").write_text(html, encoding="utf-8")
    print("💬 community: /community/ (텔레그램 + 브리핑별 댓글 예정)")
