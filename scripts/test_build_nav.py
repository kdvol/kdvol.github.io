    ...
    def test_sitewide_nav_is_compact_mobile_safe_and_idempotent(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                self.assertIn('<section class="nav-menu" aria-label="추가 메뉴"><a href="/saved/">내가 모은 글</a><a href="/cardnews/">카드뉴스</a>', morning)
                self.assertIn('<a href="/talk/">순살톡</a>', morning)
                self.assertIn('<a href="/school/" class="nav-mobile-only">스쿨</a>', morning)
                self.assertIn('<a href="/collab/" class="nav-mobile-only biz">협업 문의</a>', morning)
